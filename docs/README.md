...
Everywhere below boils down to the same three pieces:
...
3. **`data/`** - your actual files, thumbnails, and the sqlite database. This is the only
   directory that matters for backup/restore/external-drive purposes - the code and config can
   always be re-cloned/re-generated, your photos and videos can't.

Where `data/` and `config/` physically live on disk is controlled entirely by two lines in your
`.env` file (`SCLOUD_DATA_DIR` and `SCLOUD_CONFIG_DIR`), which get mounted into the container as
volumes. Moving storage to another disk is just changing those two paths and restarting the
container - never a code change, never a rebuild.
